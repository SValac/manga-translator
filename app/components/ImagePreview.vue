...
const { isOverDropZone } = useDropZone(dropZoneRef, {
  onDrop(files) {
    if (files?.[0]) store.setImage(files[0]);
  },
...
<template>
  <div ref="dropZoneRef" class="relative flex h-full items-center justify-center overflow-hidden transition-colors" :class="isOverDropZone ? 'bg-primary/5' : 'bg-muted/30'">
    ...
    <!-- Image -->
    <img v-if="store.image" :src="store.image.objectUrl" :alt="store.image.name" class="max-h-full max-w-full object-contain p-2">
    ...
    <div v-else class="flex flex-col items-center gap-4 text-center">
      ...
      <UButton icon="i-lucide-upload" label="Choose image" variant="soft" @click="inputRef?.click()" />
